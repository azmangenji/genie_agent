...
**Trigger ALSO fires on chain leaf inputs (MANDATORY).** When a `new_logic_dff` chain leaf input resolves (via fenets rename_map or RTL form) to a bus-bit `<bus>[<N>]` whose corresponding parent-scope position in a child instance's `.<bus>({...})` concat is `UNCONNECTED_*`, treat the chain leaf identically: emit `unconnected_rewires` for the parent slot AND apply the EXCEPTION below to wire the child's internal driver up. The trigger is NOT limited to direct DFF.D = UNCONNECTED — chain inputs that LOGICALLY resolve to an UNCONNECTED parent slot must be rewired the same way. Detection: for each chain leaf in the host module, scan child instance port-bus connections in the host's gate-level body; if the leaf's bus[bit] position lands on `UNCONNECTED_*`, run the EXCEPTION algorithm. Without this, chain inputs that look "resolved" via FM rename map (e.g. `oQ_UmcCfgEco_UmcCfgEco[1]`) are actually invisible at host scope because the parent DECLINED to wire them — Step 5 catches as `INPUT_UNDRIVEN`.
