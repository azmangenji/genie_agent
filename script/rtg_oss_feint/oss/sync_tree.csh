...
# Updated: Added P4 branch auto-detection from p4File parameter (OSS version)
set refDir = $1
set ip = $2
set CL = $3
set tag = $4
...
if ($p4file_count > 0) then
    # Example: //depot/oss8_0/branches/OSS_BRANCH_NAME/...
    # Extract: OSS_BRANCH_NAME
...
if ($refdir_count  == 0 ) then
     ...
     # Use branch from p4File if provided, otherwise use default
     if ("$branch_name" != "") then
...
if ($p4_mkwa_ok == 1 && $config_id_ok == 1) then
    ...
    echo "ERROR: Cannot proceed - sync verification failed"
    echo "#text#" >> $source_dir/data/${tag}_spec
    echo " Workspace sync verification: FAILED" >> $source_dir/data/${tag}_spec
    echo " Please check the sync logs and retry" >> $source_dir/data/${tag}_spec
...
echo "#text#" >> $source_dir/data/${tag}_spec
echo " The following $refdir_name has been created" >> $source_dir/data/${tag}_spec
