...
my $base_dir = $ARGV[0] || die "Usage: $0 <base_dir> <tile_name> <error_filter> [output_dir]\n";
my $tile_name = $ARGV[1] || die "Usage: $0 <base_dir> <tile_name> <error_filter> [output_dir]\n";
...
my ($dft_status, $dft_errors, $dft_warnings, $dft_waivers, $dft_filtered, $dft_unfiltered, $dft_filtered_file) = check_dft($dft_file, $error_filter, $tile_name, $output_dir);
...
sub check_dft {
    my ($file, $filter_file, $tile, $out_dir) = @_;
    return ("Not_Complete", 0, 0, 0, 0, 0, "") unless ($file && -e $file);

    # Read filters with section support (tile-based)
    my @patterns;
    my @pattern_texts;
    if ($filter_file && -e $filter_file) {
        open(my $ffh, '<', $filter_file);
    ...
    my ($errors, $warnings, $waived) = (0, 0, 0);
    ...
    while (my $line = <$fh>) {
        ...
        if ($line =~ /^\[[\dA-F]+\]/) {
            if ($line =~ /\s+(Error|ERROR)\s+/) {
                ...
                $unfiltered++ unless $matched;
            }
            $warnings++ if $line =~ /\s+(Warning|WARNING)\s+/;
        }
    }
    close($fh);
    ...
    if ($out_dir && @filtered_lines > 0) {
        $filtered_file = "$out_dir/spg_dft_filtered_${tile}.txt";
        ...
        if ($out_fh) {
            ...
            print $out_fh "=" x 70 . "\n";
            print $out_fh "Tile: $tile\n";
            print $out_fh "Report: $file\n";
            print $out_fh "Filter: $filter_file\n";
            print $out_fh "Total Filtered: $filtered\n";
            print $out_fh "=" x 70 . "\n\n";
    ...
    return ("Complete", $errors, $warnings, $waived, $filtered, $unfiltered, $filtered_file);
}
