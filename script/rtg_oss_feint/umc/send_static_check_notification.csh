#!/bin/tcsh
# Script to send email notification when static check completes
...
echo "========================================================================"
echo "Sending static check notification email..."
echo "Tag: $tag"
echo "Tile: $tile_name"
...
if (-f "${source_dir}/tasksModel.csv") then
    set mail_body = `python ${source_dir}/py/readTask.py --tasksModelFile ${source_dir}/tasksModel.csv --tag $tag --item mailBody |& grep -v "Traceback" | tail -1`
endif
...
if ("$recipients" == "") then
    ...
    set sender_email = ""
    if (-f "${source_dir}/tasksModel.csv") then
        set sender_email = `python ${source_dir}/py/readTask.py --tasksModelFile ${source_dir}/tasksModel.csv --tag $tag --item sender |& grep -v "Traceback" | tail -1`
    endif
...
if ("$passed_ip_name" != "") then
    ...
    if (-f "${source_dir}/tasksModel.csv") then
        set ip_name = `python ${source_dir}/py/readTask.py --tasksModelFile ${source_dir}/tasksModel.csv --tag $tag --item ip |& grep -v "Traceback\|Error\|No such file" | tail -1`
    endif
...
if ("$recipients" != "") then
    ...
    if ($mail_exit_status == 0) then
        echo "✓ Static check notification email sent successfully to: $recipients"
    else
        echo "✗ Failed to send static check notification email (exit code: $mail_exit_status)"
    endif
else
