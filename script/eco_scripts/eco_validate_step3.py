...
def main():
    ...
    # ── 28. INPUTS-PER-STAGE-MUST-MATCH-RENAME-MAP ──────────────────────────
    # For every entry that has `inputs_per_stage`, look up the logical signal
    # in eco_fenets_rename_map.json. If a rename_map entry exists, the studier
    # MUST use the rename_map's per-stage values verbatim (Path #1 in
    # eco_netlist_studier.md §0a). Path #3 (module-body grep) is only legal
    # when no rename_map entry exists.
    #
    # Failure mode this guards against: studier ignores Path 1, jumps to
    # Path 3 (module-body grep), and picks a logically-equivalent but
    # physically-distinct net (a common symptom is grabbing one .Qn output
    # of a multi-bit register replica when synthesis has duplicated the
    # original RTL flop). FM treats the two nets as different cone inputs
    # and reports the consuming compare point as not equivalent, even though
    # the wires carry the same logical value.
    #
    # Note: if Check 23/24 already ran, `rmap`/`rmap_keys` may be set; otherwise
    # load defensively.
    if 'rmap' not in dir() or 'rmap_keys' not in dir():
        _rmap_path = args.study.replace('_eco_preeco_study.json',
                                        '_eco_fenets_rename_map.json')
        if os.path.isfile(_rmap_path):
            try:
                rmap = json.loads(Path(_rmap_path).read_text())
                rmap_keys = set(k for k in rmap.keys() if k != '_metadata')
            except Exception:
                rmap, rmap_keys = {}, set()
        else:
    ...
    if rmap_keys:
        # Each entry's `inputs_per_stage` carries all 3 stage values, and the
        # study keeps a near-identical entry under each top-level stage key, so
        # iterating outer stages would emit the same finding 3×. Dedupe by
        # (output_net or signal_name) + target stage.
        seen28 = set()
        for stage in ('Synthesize', 'PrePlace', 'Route'):
            for e in study.get(stage, []):
                ips = e.get('inputs_per_stage') or {}
                if not ips:
                    continue
                # Logical signal = the Synthesize value (RTL name; what the gate
                # was designed to consume before P&R renamed it).
                logical = ips.get('Synthesize')
                if not logical or logical.startswith(("1'b", "0'b")):
                    continue
                # Find rename_map entry by exact match or scope-suffix match.
                # rename_map keys are either scoped paths (e.g. '<scope>/<sig>')
                # or bare signal names for Cat-1 entries.
                rkey = None
                if logical in rmap_keys:
                    rkey = logical
                else:
                    for k in rmap_keys:
                        if k.endswith('/' + logical):
                            rkey = k
                            break
                if rkey is None:
                    continue  # Path #3 (grep) is legitimate — no map entry
                rentry = rmap[rkey]
                if not isinstance(rentry, dict):
                    continue
                inst_id = (e.get('output_net')
                           or e.get('cell_instance_name')
                           or e.get('signal_name', '?'))
                for s in ('PrePlace', 'Route'):
                    studier_val = ips.get(s)
                    rmap_val = rentry.get(s)
                    if not studier_val or not rmap_val:
                        continue
                    # Exact-match rule. The rename_map value is the FM-anchored
                    # equivalence; any deviation can fail FM cone-match even
                    # when the wires hold the same logical value.
                    if studier_val != rmap_val:
                        dedupe_key = (inst_id, s)
                        if dedupe_key in seen28:
                            continue
                        seen28.add(dedupe_key)
                        issues.append(
                            f"HIGH/28-INPUTS-PER-STAGE-VS-RENAME-MAP: entry "
                            f"{inst_id!r} stage={s} inputs_per_stage[{s}]="
                            f"{studier_val!r} contradicts rename_map[{rkey!r}]"
                            f"[{s}]={rmap_val!r}. Studier bypassed Path #1 "
                            f"(rename_map verbatim) and used Path #3 (module-"
                            f"body grep). Rename_map is the authoritative "
                            f"per-stage equivalence anchor — module-body grep "
                            f"may pick a logically-equivalent but physically-"
                            f"distinct net (e.g. .Q3 of an MBR replica vs the "
                            f"FM-anchored CTS inverter), causing FM cone-"
                            f"unmatched failures. Update studier to honor "
                            f"eco_netlist_studier.md §0a Path #1.")
